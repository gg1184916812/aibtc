        ...
        const rowsHtml = forexPairs.map(pair => {
            ...
            return `
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${(pair.spread / 10).toFixed(1)} 點</td>
                    <td class="px-6 py-4 whitespace-nowrap text-right">
                        <button class="bg-blue-600 text-white py-1 px-3 rounded-md text-sm font-medium hover:bg-blue-700">交易</button>
                        <button class="bg-gray-600 text-white py-1 px-3 rounded-md text-sm font-medium hover:bg-gray-700 details-btn" data-symbol="${pair.name}">詳情</button>
                    </td>
                </tr>
            `;
        }).join('');
...
document.addEventListener('click', function(e) {
    ...
    if (e.target.classList.contains('bg-blue-600') && e.target.textContent === '交易') {
