        ...
        try {
            ...
            history.forEach(item => {
                ...
                itemElement.innerHTML = `
                    ...
                    <p class="text-sm mt-1"><span class="font-semibold">利潤:</span> ${typeof totalProfit === 'number' ? totalProfit.toLocaleString('en-US', { style: 'currency', currency: 'USD' }) : '$0.00'}</p>
                `;
    ...
    function showDetail(item) {
        try {
            ...
            detailSummary.innerHTML = `
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">策略</p><p class="font-bold">${item.strategy_name || 'N/A'}</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">市場</p><p class="font-bold">${marketName}</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">引擎</p><p class="font-bold text-${engineType === 'enhanced' ? 'green' : 'yellow'}-600">${engineType === 'enhanced' ? '🚀 增強' : '⚠️ 舊版'}</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">毛利潤</p><p class="font-bold">${totalProfit.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">點差成本</p><p class="font-bold text-red-600">-${spreadCosts.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">淨利潤</p><p class="font-bold ${totalProfit - spreadCosts >= 0 ? 'text-green-600' : 'text-red-600'}">${(totalProfit - spreadCosts).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">最大回撤</p><p class="font-bold">${maxDrawdown}%</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">勝率</p><p class="font-bold">${winRate}%</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">總交易次數</p><p class="font-bold">${totalTrades}</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">盈利次數</p><p class="font-bold">${wins}</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">虧損次數</p><p class="font-bold">${losses}</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">最大風險</p><p class="font-bold">${maxRisk}%</p></div>
                <div class="p-3 bg-gray-50 rounded"><p class="text-xs text-gray-500">最大手數</p><p class="font-bold">${maxLot}</p></div>
            `;
    ...
    function displayTradeLog(tradeLog) {
        try {
            ...
            if (!parsedTrades || parsedTrades.length === 0) {
                detailLog.innerHTML = '<h4 class="text-lg font-semibold mt-6 mb-2">交易日誌</h4><p class="text-gray-500">Tidak ada trade yang tercatat.</p>';
                return;
            }

            let logHtml = '<h4 class="text-lg font-semibold mt-6 mb-2">交易日誌 (最近 ' + Math.min(20, parsedTrades.length) + ' 筆)</h4>';
            ...
            trades.forEach(trade => {
                ...
                logHtml += `
                    <p class="mb-1">
                        ...
                        入場: ${parseFloat(entry).toFixed(4)} | 
                        出場: ${parseFloat(exit).toFixed(4)} | 
                        盈虧: <span class="${profitClass}">${parseFloat(profit).toFixed(2)}</span> | 
                        原因: ${reason}
                    </p>
                `;
            });
            ...
            detailLog.innerHTML = '<h4 class="text-lg font-semibold mt-6 mb-2">交易日誌</h4><p class="text-red-500">Error menampilkan trade log.</p>';
        }
    }
